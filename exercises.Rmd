...
data("faithful")
# Basic scatterplot
ggplot(data = faithful, # data is global to ggplot (also mapping)
       mapping = aes(x = eruptions, 
                     y = waiting)) + 
  geom_point()
# Data and mapping can be given both as global (in ggplot()) or per layer
ggplot() + # DATA is not global to ggplot
...
```{r}
ggplot(mpg) + 
  geom_jitter(aes(x = class, y = hwy), width = 0.2)+stat_summary(aes(x=class,y=hwy),fun = mean,geom="point",color='red',size=5)
```
...
```{r}
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy, colour = cty, size = cty))+guides(colour='legend')
```
...
```{r}
mpg %>% group_by(drv) %>% summarise( count = n())
...
#solution 2
ggplot(mpg) + 
  geom_point(aes(x = hwy, y = displ))+coord_trans(y = "log")
...
p2 <- ggplot(mtcars[mtcars$gear == 4,]) + 
  geom_point(aes(x = disp, y = mpg))
p1 + p2 &   scale_y_continuous(limits = c(0, 40))
...
ggplot(mpg2) + 
  ...
  transition_states(name) + ease_aes(y='cubic-in-out')
```
...
**A package to highlight specific opints/areas of a plot. Combined with ggforce can help to direct the eye of the viewer**
